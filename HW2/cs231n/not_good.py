...
class RegressionTrainer(object):
    ...
    """
        ...
        for it in range(int(num_iters)):
            if it % 10 == 0:  print('starting iteration {:d}'.format(it))

            # get batch of data
            if sample_batches:
                #batch_mask = np.random.choice(N, batch_size)
                batch_mask = np.random.permutation(N)[0:batch_size]
            ...
            if first_it or epoch_end or acc_check:
                ...
                scores_train, w = loss_function(X_train_subset, model)
                #print("scores trian", scores_train[:4],y_train_subset[:4])
                train_err = 0.5*np.sum((scores_train - y_train_subset)**2)
                for n in w:
                    train_err+=0.5*reg*np.sum(n**2)
                train_err=train_err/X_train_subset.shape[0]
                train_acc_history.append(train_err)
                # evaluate val accuracy
                scores_val, w = loss_function(X_val, model)
                val_err = 0.5*np.sum((scores_val - y_val)**2)
                #print("scores val", scores_val[:4], y_val[:4])
                for n in w:
                     val_err+=0.5*reg*np.sum(n**2)
                val_err=val_err/X_val.shape[0]
                val_acc_history.append(val_err)
                ...
                if verbose:
                    print('Finished epoch {:d} / {:d}: cost {:f}, train: {:f}, val {:f}, lr {:e}'.format(epoch, num_epochs, cost, train_err, val_err, learning_rate))


        print('finished optimization. best validation accuracy: {:f}'.format(best_val_acc))
